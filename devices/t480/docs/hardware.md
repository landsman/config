...
Windows-side config ever needs versioning it wants its own mechanism, not this
one.

Windows sharing the disk is worth remembering when touching boot config: the
`GRUB_DEFAULT` pin (see
[known issues](issues/internal-panel-black-on-resume.md)) selects a specific
kernel entry by title, so a Windows update that reorders or rewrites the EFI
boot entries can invalidate it. Re-check `uname -r` after one.
