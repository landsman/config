...
KWin has lost its DRM device — which is why nothing that talks _to_ KWin can
repair it.
...
**12x higher** on -28 than on -22. It is not _absent_ on -22 (24 occurrences
across 60 resumes) — just rare enough to live with. That distinction matters:
the upstream report is framed as a frequency regression, not a new failure,
because the errors go back to 7.0.0-15.

Corroborated by
[Framework 13 / Intel / Ubuntu 26.04](https://community.frame.work/t/ubuntu-26-04-suspend-resume-crash-on-kernel-7-0-0-27/83394)
— _"Reverting back to 7.0.0-22 fixed the issue for me."_ Symptom overlaps
...
a home** — 7.0.0-22 misses later kernel security fixes. Retest newer kernels
...
- **mesa** — `libgbm1` jumped 25.2.8 → 26.0.3 on 2026-05-18, the exact day the
  ...
  `KWIN_DRM_USE_MODIFIERS=0` made the failure rate _worse_. That date is the
  26.04 release upgrade, when mesa, kernel and KWin all moved at once.
- **`i915 ... *ERROR* Atomic update failure on pipe A`** — vblank-timing
  warnings (missed flip deadline), cosmetic. Still occur with PSR disabled via
  the kernel cmdline, and occur when everything works. Not to be confused with
...
- **PSR** (`i915.enable_psr=0`) — plausible, unproven, did not fix it. Kept as a
  harmless mitigation; see
  [`system/etc/modprobe.d/i915-no-psr.conf`](../../system/etc/modprobe.d/i915-no-psr.conf).
